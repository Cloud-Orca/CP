...
export default class SharepointComp extends LightningElement {
        ...
        try{
            ...
            if(driveItemList){
                driveItemList.forEach(function(driveItem){
                    ...
                    if(!driveItem.isPublic && !isUserInSecureGroup) {
                        return;
                    }
    ...
    async handleItemClick(event) {
        ...
        if (selectedItem && selectedItem.isFolder) {
            ...
            try{
                this.isSpinner = true;
                console.log(this.recordId);
                let driveItemList = await getChildrenFromDriveId({driveItemId: selectedItem.id, recordId: this.recordId, isPublic: selectedItem.isPublic});
                console.log(driveItemList);
    ...
    async uploadDocumentToRecord(){
        ...
        const selectedItem = this.rootItems.find(item => item.id === this.driveItemId);
        console.log(selectedItem.isPublic);
        this.isSpinner = true;
        try{
            ...
            if(isSuccess){
                if(this.driveItemId){
                    let driveItemList = await getChildrenFromDriveId({driveItemId: this.driveItemId, recordId: this.recordId, isPublic: selectedItem.isPublic});
                    console.log(driveItemList);
